...
/* Diverse the elements of the Sqlist */
void diverse(Sqlist *L)
{
...
/* Delete the duplicated elements of the Sqlist */
...
int main() 
{
    ...
    {
        int index = locateelem(B, A.elem[i]);
        if(index != -1) 
        {
            delete(&A, i+1);
            i--;
    ...
    listprint(A);
    delsame(&A);
    printf("after deleting the same element: ");
    listprint(A);
    return 0;
}